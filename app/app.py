...
st.title("SplitMyAudio by Astrokid (local env))")
...
if(current_mode == ProcessingMode.SINGLE):
    with st.form("single_mode"):
        ...
        if st.form_submit_button("Split"):
            ...
            if(selected_music == None or select_stems == None):
                ...
                st.session_state.output_files = []
                with st.spinner('We are spliting your file, might take a couple minutes...'):
    ...
    with st.container():
        st.subheader("Splited files will appear here:")
